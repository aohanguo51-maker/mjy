...
exports.main = async (event, context) => {
  ...
  if (action === 'delete') {
    ...
    try {
      ...
      if (!firstDoc(docRes)) return { code: 404, msg: '记忆不存在' };
      if (firstDoc(docRes).ownerId !== uid) return { code: 403, msg: '无权删除他人记忆' };
  ...
  if (action === 'update') {
    ...
    try {
      const docRes = await memCol.doc(memoryId).get();
      if (!firstDoc(docRes)) return { code: 404, msg: '记忆不存在' };
      if (firstDoc(docRes).ownerId !== uid) return { code: 403, msg: '无权修改他人记忆' };
